...
app.get('/health', (req, res) => {
  res.json({ status: 'OK', message: 'BookGX Backend is running', timestamp: new Date().toISOString() });
});
...
app.post('/api/fetchSheets', async (req, res) => {
  try {
    ...
    if (!connection || !connection.clientEmail || !connection.privateKey || !connection.projectId) {
      return res.status(400).json({ 
        error: 'Service account credentials are required (clientEmail, privateKey, projectId)' 
      });
    }
    ...
    if (error.code === 403) {
      errorMessage = 'Permission denied - ensure service account has access to the sheet';
...
app.post('/api/fetchHeaders', async (req, res) => {
  try {
    ...
    if (error.code === 403) {
      ...
      errorMessage = 'Sheet not found - check the sheet name and spreadsheet ID';
